...
**Phase 9 in progress** — PWA/safe-area/update-notification fixes, the reconciliation/stale-signal bugs, the autopay race condition, the account-card sizing/content fix, and this round of real-device visual bugs are all shipped; the `updateDoc()`→`setDoc()`, per-item error isolation, and Confirm double-tap-guard backlog items below remain deferred, along with fully exercising authenticated-page *functionality* on-device (visual layout has now been spot-checked via real user screenshots; functional testing — Plaid flows, editing, etc. — is still the user's to report back on).
