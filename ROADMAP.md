...
- [ ] **NEXT SESSION — resume here.** Disconnect and relink Discover fresh to test the corrected history-window understanding and pick up Liabilities.
  ...
  - Done when: Discover is disconnected via Accounts → Disconnect, then reconnected via "Connect bank account" with the custom start date set to **01/01/2026** again, and the resulting transaction count (check Transactions page, "All time" filter) is compared against the previous single-transaction result.
  - Not yet done: this requires interactively completing Plaid Link with real Discover credentials in the browser — genuinely needs the user, not something automatable. **If the count is still 1 after a truly fresh Item**, that's real, solid confirmation of an institution-side limit (not a bug) and this item can be marked verified as "confirmed institution limit, no further action possible." If more transactions appear, that confirms the fix worked and the item can be marked verified as such. Either way, also check whether the account card now shows Minimum Payment / Due Date / statement info (confirms Liabilities is live on the new Item).
