...
export class Transactions {
  ...
  // Broken/blank transactions (from the earlier save-race bug): a non-finite amount,
  // or a transfer missing one of its accounts. These never held valid data and show
  // as "Untitled / $NaN". Surfaced for a one-click reviewed cleanup.
  invalidTransactions = computed(() =>
    this.txService.transactions().filter(t =>
      !!t.id && (
        !Number.isFinite(t.amount) ||
        (t.type === 'transfer' && (!t.fromAccountId || !t.toAccountId))
      )
    )
  );
  invalidConfirmOpen = signal(false);

  isFiniteAmount(n: number): boolean {
    return Number.isFinite(n);
  }

  async confirmDeleteInvalid() {
    if (this.bulkSaving()) return;
    const ids = this.invalidTransactions().map(t => t.id).filter((id): id is string => !!id);
    if (ids.length === 0) { this.invalidConfirmOpen.set(false); return; }
    this.bulkSaving.set(true);
    try {
      await this.txService.removeMany(ids);
      this.toastService.success(`Removed ${ids.length} blank/invalid transaction(s).`);
    } catch (err) {
      this.toastService.error('Could not remove those transactions.');
    } finally {
      this.bulkSaving.set(false);
      this.invalidConfirmOpen.set(false);
    }
  }

  // Duplicate bank transactions: when several docs share one Plaid transaction id
  // (unique per real charge), one is your edited/merged entry (doc id !== the Plaid
  // id — carries your merchant/notes/category) and the other is the raw bank-synced
  // row (doc id === the Plaid id). We keep YOUR version and remove the bank duplicate.
  // Exact match on the Plaid id, so identical-looking transactions (e.g. same-day
  // $21.82 Zelles) are never confused. This list is the bank rows that get removed.
  duplicateBankCopies = computed<Transaction[]>(() => {
    const txs = this.txService.transactions();
    const byPtid = new Map<string, Transaction[]>();
    for (const t of txs) {
      if (!t.id || !t.plaidTransactionId) continue;
      const arr = byPtid.get(t.plaidTransactionId) ?? [];
      arr.push(t);
      byPtid.set(t.plaidTransactionId, arr);
    }
    const toRemove: Transaction[] = [];
    for (const [ptid, group] of byPtid) {
      if (group.length < 2) continue;
      // Keep your edited copy (id !== ptid); if there's more than one, keep the first.
      // If there's no edited copy at all (all raw), keep one raw row.
      const keep = group.find(t => t.id !== ptid) ?? group[0];
      for (const t of group) if (t !== keep && t.id) toRemove.push(t);
    }
    return toRemove;
  });
  dedupeConfirmOpen = signal(false);

  async confirmDedupe() {
    if (this.bulkSaving()) return;
    const ids = this.duplicateBankCopies().map(t => t.id).filter((id): id is string => !!id);
    if (ids.length === 0) { this.dedupeConfirmOpen.set(false); return; }
    this.bulkSaving.set(true);
    try {
      await this.txService.removeMany(ids);
      this.toastService.success(`Removed ${ids.length} bank duplicate(s). Kept your edited version of each.`);
    } catch (err) {
      this.toastService.error('Could not remove the duplicates.');
    } finally {
      this.bulkSaving.set(false);
      this.dedupeConfirmOpen.set(false);
    }
  }
