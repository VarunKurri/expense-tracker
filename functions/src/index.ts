...
/**
 * Recover missing transactions for one item by re-pulling its full history from an
 * EMPTY (local) cursor and re-creating only the transactions whose Firestore doc no
 * longer exists (e.g. accidentally deleted). Deliberately different from syncItem:
 *   - starts from a fresh cursor to see everything Plaid still has, but
 *   - NEVER writes that cursor back to the item (the real incremental cursor is left
 *     alone, so normal webhook syncs keep working from where they were), and
 *   - only writes docs that are MISSING (existing docs — including any the user has
 *     edited/re-encrypted — are left untouched, never overwritten).
 */
async function recoverItem(
  ...
  const db = getFirestore();
  const item = itemDoc.data();
  if (!item) return { recovered: 0, scanned: 0 };
  const accessToken = decryptToken(item.accessToken as EncryptedValue, tokenEncKeyValue);
  const txCol = db.collection(`users/${uid}/transactions`);
 ...
 * linked item's full history and re-create only the docs that are missing.
 * Non-destructive (see recoverItem) — safe to run more than once.
 */
export const recoverDeletedTransactions = onCall(
  { secrets: [plaidClientId, plaidSecret, tokenEncKey] },
  async (request) => {
    if (!request.auth) {
      throw new HttpsError('unauthenticated', 'You must be signed in.');
    ...
    if (!publicKey) {
      throw new HttpsError(
        'failed-precondition',
        'Unlock the app once to set up your encryption keys before recovering.',
      );
    }
    const client = getPlaidClient(plaidClientId.value(), plaidSecret.value(), plaidEnv.value());
    const db = getFirestore();
    ...
    const results: { itemId: string; institutionName: string; recovered: number; scanned: number }[] = [];
    let totalRecovered = 0;
    for (const itemDoc of itemsSnap.docs) {
      const institutionName = (itemDoc.data()?.institutionName as string) || 'Your bank';
      try {
        const r = await recoverItem(uid, itemDoc, client, tokenEncKey.value(), publicKey);
        totalRecovered += r.recovered;
        results.push({ itemId: itemDoc.id, institutionName, ...r });
        console.log(`recover ${institutionName} (${itemDoc.id}): scanned ${r.scanned}, recovered ${r.recovered}`);
      } catch (err: any) {
        console.error(`recover failed for item ${itemDoc.id}:`, err?.response?.data || err);
        results.push({ itemId: itemDoc.id, institutionName, recovered: 0, scanned: 0 });
      }
    }
    console.log(`recover total: ${totalRecovered} across ${results.length} item(s)`);
    return { totalRecovered, results };
  },
);
