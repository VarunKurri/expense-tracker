...
window.Icon = function Icon({ name, size = 16, stroke = 1.75 }) {
  const paths = {
    ...
    vault: 'M3 5a2 2 0 012-2h10a2 2 0 012 2v10a2 2 0 01-2 2H5a2 2 0 01-2-2V5zm7 3a2 2 0 100 4 2 2 0 000-4zm0 0V6m0 6v2M7 10H5m10 0h-2',
    ...
    chart: 'M3 17V8m5 9V4m5 13V10m5 7V6',
...
window.Sidebar = function Sidebar({ active, onNav, data }) {
  const main = [
    { id: 'home',   label: 'Home',         icon: 'home' },
    { id: 'accts',  label: 'Accounts',     icon: 'accounts' },
    { id: 'tx',     label: 'Transactions', icon: 'tx' },
    { id: 'bills',  label: 'Bills',        icon: 'bills', badge: 3 },
  ...
  return (
    <aside className="sidebar">
      ...
      <nav className="nav">
        ...
        {main.map(i => (
          <div key={i.id} className={'nav-item' + (active === i.id ? ' active' : '')} onClick={() => onNav(i.id)}>
            <Icon name={i.icon} />
            <span>{i.label}</span>
      ...
      <div className="user-card" onClick={() => onNav('profile')}>
        <div className="avatar">{data.user.avatar}</div>
        <div style={{minWidth:0, flex:1}}>
          <div className="user-name">{data.user.name}</div>
          <div className="user-handle">{data.user.handle}</div>
        </div>
      </div>
    </aside>
  );
...
window.Topbar = function Topbar({ title, onQuickAdd, onNav }) {
  return (
    <div className="topbar">
      <div className="topbar-title">{title}</div>
      <div className="search">
        <Icon name="search" size={14}/>
        <input placeholder="Search transactions, merchants, bills…"/>
